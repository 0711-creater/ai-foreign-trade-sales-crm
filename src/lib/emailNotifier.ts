...
function buildNotificationText({ inquiryRecord, aiAnalysis, crmDetailUrl }: InquiryNotificationInput) {
  return [
    ...
    `Lead Priority: ${aiAnalysis.leadPriority}`,
    `Recommended Follow-up Time: ${aiAnalysis.recommendedFollowUpTime}`,
    `Sales Strategy: ${aiAnalysis.salesStrategy}`,
    `Follow-up Priority: ${inquiryRecord.followUpPriority}`,
    `Follow-up Due At: ${inquiryRecord.followUpDueAt}`,
    `Follow-up Stage: ${inquiryRecord.followUpStage}`,
    `Next Action: ${inquiryRecord.nextAction}`,
